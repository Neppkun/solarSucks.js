...
class SlashHandler {
  ...
  #load(path, commands, guildCommands) {
    if (!existsSync(path))
    ...
    this.#loadCommands(join(this.#cwd, path), commands, guildCommands);
  }

  #loadCommands(path, commands, guildCommands) {
    if (statSync(path).isDirectory())
      for (const entry of readdirSync(path))
        this.#loadCommands(join(path, entry), commands, guildCommands);
    else if (path.endsWith('.js')) {
      const parts = path.split(delimiter);
      const entry = parts.pop();
      
      this.#loadCommand(parts.join(delimiter), entry, commands, guildCommands);
    }
  }

  #loadCommand(path, file, commands, guildCommands) {
    try {
